  ...
  Widget build(BuildContext context) {
    return BlocBuilder<PodcastSettingsCubit, PodcastSettingsState>(
      ...
      if (settingsState is PodcastSettingsLoaded) {
        final PodcastFilterSettingsEntity currentFilterSettings =
            settingsState.settings;
        String? dropdownValue;
        if (currentFilterSettings.showOnlyRead) {
          dropdownValue = 'Read';
        } else if (currentFilterSettings.showOnlyUnfinished) {
          dropdownValue = 'Unfinished';
        } else if (currentFilterSettings.showOnlyFavorites) {
          dropdownValue = 'Favorites';
        } else if (currentFilterSettings.showOnlyDownloaded) {
          dropdownValue = 'Downloaded';
        } else {
          if (!currentFilterSettings.showOnlyRead &&
              !currentFilterSettings.showOnlyUnfinished &&
              !currentFilterSettings.showOnlyFavorites &&
              !currentFilterSettings.showOnlyDownloaded) {
            dropdownValue = 'All';
          } else {
            dropdownValue = null;
          }
        ...
        return DropdownButtonHideUnderline(
          child: DropdownButton<String>(
            ...
            iconEnabledColor: const Color(0xFFF28F3B),
            dropdownColor: const Color(0xFF202531),
            style: Theme.of(context).textTheme.bodyLarge,
            borderRadius: BorderRadius.circular(10),
            alignment: Alignment.center,
            value: dropdownValue,
            hint: Text("All", style: Theme.of(context).textTheme.bodyLarge),
            isDense: true,
